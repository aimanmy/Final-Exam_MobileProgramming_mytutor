...
if ($signed === $data['x_signature']) {
    if ($paidstatus == "Success"){ //payment success
        ...
        $sqpupdatecart = "UPDATE `tbl_subscriptions` SET `subscription_status`='paid',`receipt_id`='$receiptid' WHERE user_email='$email' AND subscription_status IS NULL";
        if ($conn->query($sqlinsertpayment) && $conn->query($sqpupdatecart)){
            $sqlselectcart="SELECT * FROM tbl_subscriptions WHERE receipt_id = '$receiptid'";
            $result = $conn->query($sqlselectcart);
            if ($result->num_rows > 0) {
                while ($rows = $result->fetch_assoc()) {
                    $subid = $rows['subject_id'];
                    $subscriptionqty = $rows['subscription_qty'];
                    $updateproductsqty = "UPDATE tbl_subjects set subscription_qty = (subscription_qty-$subscriptionqty) WHERE subject_id = '$subid'";
